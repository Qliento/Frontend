...
const AnaliticCard = () => {
  ...
  useEffect(() => {
    window.Ya.share2("ya", {
      theme: {
        services: "vkontakte,facebook,telegram,linkedin,twitter",
        size: "l",
      },
    });
  });
  ...
  let textCountry = textCountry1 && textCountry1.join(" ");
  return (
      ...
      <div className={st.card_content}>
        <div className={st.card_img}>
          ...
          {data && data.research && (
            <div className={st.research_info}>
              <Link
                to={`/market-research-detail/${
                  data.research && data.research.id
                }`}
              >
                <div className={st.research_info_header}>
                  <span>
                    {data.research.author && data.research.author.logo}
                  </span>
        ...
        <div className={st.card_description}>
          <span>
            ...
            {(language == 1 || language == undefined) && (
              <span>
                {data && month[Number(data.date.match(/\d+/g)[1] - 1)]}{" "}
              </span>
            )}
            {language == 2 && (
              <span>
                {data && monthEng[Number(data.date.match(/\d+/g)[1] - 1)]}{" "}
              </span>
            )}
            {language == 3 && (
              <span>
                {data && monthKg[Number(data.date.match(/\d+/g)[1] - 1)]}{" "}
              </span>
            )}
